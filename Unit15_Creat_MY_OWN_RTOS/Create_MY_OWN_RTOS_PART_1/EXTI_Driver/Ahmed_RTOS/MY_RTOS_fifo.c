...
FIFO_status FIFO_PUSH(FIFO_t *FIFO,element_type item)
{
	...
	//check if FIFO is full or not
	 if((FIFO->head == ((FIFO->base) + (sizeof(element_size)*(FIFO->length))))||(FIFO->count==FIFO->length))
		 return FIFO_FULL;
	 if((FIFO->head < ((FIFO->base) + (sizeof(element_size)*((uint32_t)FIFO->length))))||(FIFO->count==FIFO->length)){
	 *(FIFO->head)=item;
	 FIFO->head++;
	 FIFO->count++;
	 }else{
...
FIFO_status FIFO_POP(FIFO_t *FIFO,element_type *item)
{
	...
	if(FIFO->count==0)
		return FIFO_empty;
	if((FIFO->tail < ((FIFO->base) + (sizeof(element_size)*((uint32_t)FIFO->length))))){
	*item=*(FIFO->tail);
	FIFO->tail++;
	FIFO->count--;
	}else{
